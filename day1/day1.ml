...
let print_int_list l = List.map string_of_int l |> String.concat "\n" |> print_string
...
let () = (open_in "input.txt") |> process_stdin int_of_string |> merge |> count_growing None |> string_of_int |> print_endline

    (*|> count_growing None |> string_of_int |> print_endline*)
